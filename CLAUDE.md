...
- **render 韌性**：zero-md 冷啟動 `render()` 偶不 resolve → `renderUntilBody` timeout-race 重試。**成功條件 `= res.body || bodyPainted()`**（`res.body` 是「原文 hash 有變」旗標、非「已渲染」；冷啟動超過 race 1200ms 時會被丟棄、此後永遠 `false`、overlay 卡住）——`bodyPainted()` 比對 `.markdown-body[data-hash]` 與 slot 原文 hash 判斷是否真的畫上。設 slot 後讓出 macrotask 再 render。詳見家族 §4.3。

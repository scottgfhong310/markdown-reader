...
| `md-tweaks.js` | `markdown-library` 那份為準（byte-identical）。共 6 份複製件；**稽核在權威版 repo**：`markdown-library` 的 `node scripts/test-md-tweaks.js`（D 項逐位元組比對這 6 份）。⚠ 該 repo 的 `sync-copies.sh` **不會推到本 app**——權威版改了要自己 `cp` 過來，沒有任何腳本會替你做 |

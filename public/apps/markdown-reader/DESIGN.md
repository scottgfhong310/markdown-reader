...
`MdTweaks.apply`：純字串、以 NUL 佔位**遮罩程式碼**後再轉換。目前五項：`**Tags**` 後的 hashtag 收成單行行內碼、**無標題的純 hashtag 區塊**也收成單行（以空行分隔的區塊整塊只有 tag 才處理；`#x` 與跳脫 `\#x` 皆吃、輸出去跳脫）、裸 `~` 補空白（避免 GFM 誤判刪除線）、CJK 全形括號緊貼 `**` 補空白（避免 marked flanking 規則認不出粗體分隔符）、`<span class="note">` 依字數補 `style="max-width: ##em"`（值＝`max(內容字數 ÷ 2 進位, 最長半行)`，最長半行僅在有手插 `<br/>` 時參與比較；不計內層標籤、已自帶 `style=` 者不動、收尾 `</span>` 數深度找）。新增微調＝加一個純函式到 `TWEAKS` 陣列。
